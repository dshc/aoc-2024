...
def part2():
    ...
    end_score = None
    while heap:
        score, loc, idir, t = heappop(heap)
        if loc == end:
            if end_score is None or end_score > score:
                end_score = score
                tiles = t
            elif end_score == score:
                tiles = tiles | t
            continue
        ...
        for i in range(len(dirs)):
            ...
            next_loc = (next_dir[0] + loc[0], next_dir[1] + loc[1])
            if idir == i:
                heappush(heap, (score + 1, next_loc, i, t | {next_loc}))
            else:
                heappush(heap, (score + 1001, next_loc, i, t | {next_loc}))
